...
2. 配置分为 `global`（全局配置）和 `local`（局部配置）
    - 全局配置：未找到局部配置时默认使用，可通过 `-c` / `--config` 覆盖默认参数
        - 路径与环境设置

            | 项目         |    类型     | 默认值              | 说明                                                                                             |
            | :----------- | :---------: | :------------------ | :----------------------------------------------------------------------------------------------- |
            | `path.src`   |    `str`    | `"~/Downloads/src"` | EPUB 源文件的存放目录或单文件。                                                                  |
            ...
            | `path.dbg`   | `str/None`  | `None`              | 调试目录，将解析后的页面数据导出为 JSON 文件以排查解析错误                                       |
        ...
        - 排版与样式处理
            ...
            | `image.ialt`  |   `bool`    | `True`     | 当行内图片带有 `alt` 属性时，是否直接输出 `alt` 文本而不显示图片（常用于处理带浊点的假名外字，如「あ゛」） |
            | `image.balt`  |   `bool`    | `False`    | 当行间图片带有 `alt` 属性时，是否直接输出 `alt` 文本而不显示图片                                           |
